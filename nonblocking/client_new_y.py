...
class Client:
    ...
    def send_message(self, user: PeerUser, msg_data_header: str, msg_data_string=""):
        ...
        message_header = f"{len(message_string):<{self.HEADER_LENGTH}}".encode(
            'utf-8')
        self.SOCKETS_LIST[user.sockets_list_index].send(
    ...
    def create_peer_connection(self, name, IP, PORT):
        ...
        self.PEERS[peer_socket] = PeerUser(IP, PORT, peer_socket, len(self.SOCKETS_LIST)-1, name)
    ...
    def establish_peer_connection(self):
        ...
        peer_user = PeerUser(peer_address[0], peer_address[1], peer_socket, sockets_list_index, "")
    ...
    def send_chat_request(self, users: list):
        request_message = f"{self.username} sent a chat request"
        is_connected, peer_socket = self.create_peer_connection(users[0],
                                                                self.SEARCH_RESULT[users[0]][0], 
                                                                self.SEARCH_RESULT[users[0]][1])
        self.send_message(self.PEERS[peer_socket],self.MESSAGE_TYPES_OUT["ChatRequest"], request_message)

    def display_search_result(self, msg_data:str):
        for line in msg_data.split('*'):
            splitted = line.split(' ')
            if len(splitted) == 3:
                print(f"User {splitted[0]} available at address {splitted[1]}:{splitted[2]}")
                ...
                for notified_socket in read_sockets:
                    if notified_socket == self.client_server_socket:
                        ...
                        if message['header'] == self.MESSAGE_TYPES_IN["SearchResult"]:
                            ...
                            pass
                        elif message['header'] == self.MESSAGE_TYPES_IN["RequestAccept"]:

                            pass
                        elif message['header'] == self.MESSAGE_TYPES_IN["RequestReject"]:
                            pass
